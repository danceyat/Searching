 ...
 * 
 * <pre>
 * CursorJoinerUtil joiner = new CursorJoinerUtil(cursorA, keyColumnsofA, cursorB,
 * 		keyColumnsofB);
 ...
 * </pre>
 */
public final class CursorJoinerUtil implements
		Iterator<CursorJoinerUtil.Result>, Iterable<CursorJoinerUtil.Result> {
	...
	/**
	 * Initializes the CursorJoinerUtil and resets the cursors to the first row.
	 * The left and right column name arrays must have the same number of
	 * columns.
	 * 
	 * @param cursorLeft
	 *            The left cursor to compare
	 * @param columnNamesLeft
	 *            The column names to compare from the left cursor
	 * @param cursorRight
	 *            The right cursor to compare
	 * @param columnNamesRight
	 *            The column names to compare from the right cursor
	 */
	public CursorJoinerUtil(Cursor cursorLeft, String[] columnNamesLeft,
	 ...
	 * 
	 * @param cursor
	 *            the cursor that contains the columns
	 * @param columnNames
		...
		if (mCompareResultIsValid) {
			...
			default:
				throw new IllegalStateException(
						"bad value for mCompareResult, " + mCompareResult);
			}
		} else {
	...
	/**
	 * Returns the comparison result of the next row from each cursor. If one
	 * cursor has no more rows but the other does then subsequent calls to this
	 * will indicate that the remaining rows are unique.
	 * <p>
	 * The caller must check that hasNext() returns true before calling this.
	 * <p>
	 * Once next() has been called the cursors specified in the result of the
	 * call to next() are guaranteed to point to the row that was indicated.
	 * Reading values from the cursor that was not indicated in the call to
	 * next() will result in undefined behavior.
	 * 
	 * @return LEFT, if the row pointed to by the left cursor is unique, RIGHT
	 *         if the row pointed to by the right cursor is unique, BOTH if the
	 *         rows in both cursors are the same.
	 */
	public Result next() {
		if (!hasNext()) {
			throw new IllegalStateException(
					"you must only call next() when hasNext() is true");
		}
		incrementCursors();
		assert hasNext();
	...
	/**
	 * Increment the cursors past the rows indicated in the most recent call to
	 * next(). This will only have an affect once per call to next().
	 */
	...
	/**
	 * Compare the values. Values contains n pairs of strings. If all the pairs
	 * of strings match then returns 0. Otherwise returns the comparison result
	 * of the first non-matching pair of values, -1 if the first of the pair is
	 * less than the second of the pair or 1 if it is greater.
	 * 
	 * @param values
	 *            the n pairs of values to compare
	 * @return -1, 0, or 1 as described above.
	 */
	private static int compareStrings(String... values) {
		if ((values.length % 2) != 0) {
			throw new IllegalArgumentException(
					"you must specify an even number of values");
		}
		...
		for (int index = 0; index < values.length; index += 2) {
			...
			int num1 = 0, num2 = 0;
			try {
				num1 = Integer.parseInt(values[index]);
				num2 = Integer.parseInt(values[index + 1]);
